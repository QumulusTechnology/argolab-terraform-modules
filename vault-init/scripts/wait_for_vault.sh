...
## Usage: wait_for_vault.sh <host> base64encode(<client_certificate>) base64encode(<client_key>) base64encode(<cluster_ca_certificate>)
...
until [ "$n" -ge 6 ]
do
   kubectl --kubeconfig $NULL_KUBECONFIG --server $HOST --client-certificate $CLIENT_CERTIFICATE_FILE --client-key $CLIENT_KEY_FILE --certificate-authority $CLUSTER_CA_CERTIFICATE_FILE -n vault wait --for=condition=Ready pod/vault-0 --timeout=600s &> /dev/null && break
   RETVAL=$?
   n=$((n+1))
   sleep 1
done
