...
module "vault_azure_ad_groups" {
  source           = "git@github.com:QumulusTechnology/terraform-vault-azuread-groups.git?ref=v1.0.1"
  auth_method      = vault_jwt_auth_backend.this.accessor
  security_enabled = true
}

# This synchronises the users from Azure AD into Vault
module "vault_azure_ad_users" {
  source      = "git@github.com:QumulusTechnology/terraform-vault-azuread-users.git?ref=v1.0.1"
  auth_method = vault_jwt_auth_backend.this.accessor
}
